...
func (c *command) Run(args []string) int {
	...
	if !csv {
		ui.Output("Checking whether provider uses packages or identifiers removed from the new SDK...")
	}
	removedPackagesInUse, removedIdentsInUse, doesNotUseRemovedPackagesOrIdents, err := CheckSDKPackageImportsAndRefs(providerPath)
	if !csv {
		if err != nil {
			log.Printf("Error determining use of removed SDK packages and idents: %s", err)
			return 1
		}
		if doesNotUseRemovedPackagesOrIdents {
			ui.Info("No imports of removed SDK packages or idents: OK.")
		} else {
			ui.Warn(fmt.Sprintf("Removed SDK packages in use: %+v", removedPackagesInUse))
			ui.Warn(fmt.Sprintf("Removed SDK idents in use: %+v", removedIdentsInUse))
		}
	}
